...
1. A process needs versioning. U always thought of versioning of a way how data was changed. Maybe that's due to may developer background who treats versioning as a log and not as a way to proof which data was there, when someone approved/signed something. Also, it's kind a obvious.
...
5. Power Platform is for automation and [not for](https://youtu.be/4TSYSO1hVC0?t=5142) business processes. There are to many workarounds required (at this point in time) 
6. Model-driven apps share the same idea with WEBCON BPS on how data impacts form elements.
